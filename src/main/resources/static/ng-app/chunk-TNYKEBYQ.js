...
function isNativeFormElement2(element) {
  let nodeName = element.nodeName.toLowerCase();
...
function isPotentiallyFocusable(element) {
  ...
  return isNativeFormElement2(element) || isAnchorWithHref(element) || element.hasAttribute("contenteditable") || hasValidTabIndex(element);
}
    ...
    return this._ngZone.runOutsideAngular(() => {
      ...
      this._previousTimeout = setTimeout(() => {
        if (!message2 || typeof message2 === "string") {
          ...
          _setInnerHtml(this._liveElement, message2, this._sanitizer);
        }
...
var AriaDescriber = class _AriaDescriber {
  ...
  describe(hostElement, message2, role) {
    if (!this._canBeDescribed(hostElement, message2)) {
      return;
    }
    const key = getKey(message2, role);
    if (typeof message2 !== "string") {
      setMessageId(message2, this._id);
      this._messageRegistry.set(key, {
        messageElement: message2,
        referenceCount: 0
      });
    } else if (!this._messageRegistry.has(key)) {
      this._createMessageElement(message2, role);
    }
  ...
  _createMessageElement(message2, role) {
    ...
    this._messageRegistry.set(getKey(message2, role), {
      messageElement,
      referenceCount: 0
    });
  }
  ...
  _canBeDescribed(element, message2) {
    ...
    const trimmedMessage = message2 == null ? "" : `${message2}`.trim();
...
function getKey(message2, role) {
  return typeof message2 === "string" ? `${role || ""}/${message2}` : message2;
}
...
var NZ_DATE_ADAPTER = new InjectionToken(typeof ngDevMode !== "undefined" && ngDevMode ? "nz-date-adapter-type" : "");
function provideNzDateAdapter(adapterClass, config) {
  const _a = config ?? {}, {
    locale
  } = _a, dateConfig = __objRest(_a, [
    "locale"
  ]);
  return makeEnvironmentProviders([adapterClass, {
    provide: NzDateAdapter,
    useExisting: adapterClass
  }, {
    provide: NZ_DATE_CONFIG,
    useValue: __spreadValues(__spreadValues({}, NZ_DATE_CONFIG_DEFAULT), dateConfig)
  }, ...locale !== void 0 ? [{
...
var DateFnsDateAdapter = class _DateFnsDateAdapter extends NzDateAdapter {
  dateLocale = inject(NZ_DATE_LOCALE, {
    optional: true
  });
  dateConfig = inject(NZ_DATE_CONFIG, {
    optional: true
  });
  constructor() {
    super();
  ...
  getMonthNames(style) {
    ...
    return Array.from({
      length: 12
    }, (_, i) => format(new Date(2024, i, 1), format$1, {
      locale: this.locale
    }));
  }
  getDateNames() {
  ...
  getDayOfWeekNames(style) {
    ...
    return Array.from({
      length: 7
    }, (_, i) => format(new Date(2024, 0, i + 1), format$1, {
      locale: this.locale
    }));
  }
  // --- Week ---
  ...
  parse(value, parseFormat) {
    if (typeof value === "string" && value.length > 0) {
      ...
      for (const currentFormat of formats) {
        ...
        const fromFormat = parse(value, formatString, /* @__PURE__ */ new Date(), {
          locale: this.locale,
  ...
  static \u0275fac = function DateFnsDateAdapter_Factory(__ngFactoryType__) {
...
var NativeDateAdapter = class _NativeDateAdapter extends NzDateAdapter {
  dateLocale = inject(NZ_DATE_LOCALE, {
    optional: true
  });
  dateConfig = inject(NZ_DATE_CONFIG, {
  ...
  constructor() {
    super();
    if (this.dateLocale !== void 0 && typeof this.dateLocale === "string") {
  ...
  parseByToken(value, formatStr) {
    ...
    let match2;
    try {
      match2 = value.match(new RegExp(`^${source}$`));
    } catch {
      return null;
    }
    if (!match2?.groups) {
    ...
    const year = match2.groups["year"] ? Number(match2.groups["year"]) : match2.groups["year2"] ? 2e3 + Number(match2.groups["year2"]) : this.today().getFullYear();
    const month = match2.groups["month"] ? Number(match2.groups["month"]) - 1 : 0;
    const day = match2.groups["day"] ? Number(match2.groups["day"]) : 1;
    const hour = match2.groups["hour"] ? Number(match2.groups["hour"]) : 0;
    const minute = match2.groups["minute"] ? Number(match2.groups["minute"]) : 0;
    const second = match2.groups["second"] ? Number(match2.groups["second"]) : 0;
    if (!inRange(month, 0, 11) || !inRange(day, 1, 31) || !inRange(hour, 0, 23) || !inRange(minute, 0, 59) || !inRange(second, 0, 59)) {
      return this.invalid();
    }
...
//# debugId=a764df12-b410-5708-b1d9-16c6e3b817bd
//# sourceMappingURL=chunk-TNYKEBYQ.js.map
