...
import { AuthDeptNode, CodeItem, ItemOption, SearchApplyOtRow, ShiftOption } from './search-apply-ot-info-list.model';
import { SearchApplyOtInfoListService } from './search-apply-ot-info-list.service';
...
const I18N_KEYS = [
  ...
  'common.fromDate', 'common.toDate', 'attSearch.empGroup', 'attSearch.shiftType', 'applyOt.otType',
  'attSearch.all', 'common.search', 'common.clearFilter', 'common.exportExcel', 'common.stt',
  'attSearch.workDate', 'attSearch.dateTypeReal', 'common.empId', 'common.empName', 'attSearch.position',
  'attSearch.fromTime', 'attSearch.toTime', 'attSearch.quantity', 'attSearch.unit', 'common.status',
  'common.remark', 'applyOt.updater', 'applyOt.updateTime', 'common.totalRows', 'common.loadFail',
...
/**
 * Bản Angular của ar/attendanceMintenance/viewSearchApplyOtInfoList.html (Thymeleaf + DataTables),
 * gọi lại nguyên các API JSON đã có sẵn (xem SearchApplyOtInfoListService) - không đổi backend. Dùng
 * nz-table (phân trang client-side, giống hệt hành vi DataTables gốc: BE trả toàn bộ danh sách theo
 * bộ lọc, không phân trang server) thay cho DataTables; nz-tree-select thay widget cây phòng ban tự
 * viết (DeptTree.js) - kiến trúc bám sát attendance-search.component.ts (cùng dùng chung
 * ArAttendanceSearchDto/endpoint gốc /api/attendanceSearch, chỉ khác /list và /otList). Xuất excel
 * làm client-side (không có endpoint export riêng ở backend - bản gốc cũng xuất client-side qua
 * DataTables Buttons).
 */
@Component({
  selector: 'app-search-apply-ot-info-list',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './search-apply-ot-info-list.component.html',
  styleUrl: './search-apply-ot-info-list.component.css',
})
export class SearchApplyOtInfoListComponent implements OnInit {
  readonly pageSizeOptions = PAGE_SIZE_OPTIONS;

  readonly rows = signal<SearchApplyOtRow[]>([]);
  ...
  constructor(
    private readonly api: SearchApplyOtInfoListService,
  ...
  exportExcel(): void {
    const headers = [
      ...
      this.i18n.t('attSearch.dateTypeReal', 'Loại ngày công'),
    ...
    const dataRows = this.rows().map((r, i) => [
      i + 1, r.arDateStr, r.dateTypeRealName, r.empId, r.localName, r.deptName, r.postGradeName,
      r.postFamilyName, r.shiftName, r.itemName, r.fromTime, r.toTime, r.quantity, r.unit,
      r.statusName, r.remark, r.updatedBy, r.updateDate,
    ]);

    const worksheet = XLSX.utils.aoa_to_sheet([headers, ...dataRows]);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'TraCuuDangKyTangCa');
    XLSX.writeFile(workbook, 'search_apply_ot_info_list.xlsx');
  }
