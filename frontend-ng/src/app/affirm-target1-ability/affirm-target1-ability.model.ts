...
/** Tương ứng với EvsAffirmTarget1Dto.java (1 dòng trong danh sách nhân viên - dùng chung endpoint
 *  /affirmTarget1/objectList với AffirmTarget1Component). */
...
/** Tương ứng với EvsParamDto.java (chỉ dùng làm tùy chọn điểm đánh giá lần 1 - paramType=ITEM). */
...
export interface EvsAffirmTarget1AbilityDetailInfo {
  seq: string;
  evsYear: string;
  localName: string;
  postGradeName: string;
...
/** Payload gửi lên POST .../affirmTarget1Ability/saveDetail|confirmDetail. */
export interface SaveDetailPayload {
  seq: string;
  resumeSeq: string;
  affirmContent: string;
  items: { itemSeq: string; evsScore1: string | null }[];
}
