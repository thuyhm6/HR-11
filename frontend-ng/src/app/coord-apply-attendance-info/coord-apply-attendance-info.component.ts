...
import { CoordApplyAttendanceInfoService } from './coord-apply-attendance-info.service';
import {
...
/** Toàn bộ key bên dưới đã có sẵn trong message.properties (namespace "cai" dùng riêng cho trang này,
 *  "ex"/"common"/"mep" dùng chung với các trang khác) - không cần thêm key mới. */
const I18N_KEYS = [
  'ex.field.empIdOrName', 'ex.placeholder.empIdOrName', 'ex.field.dept', 'ex.placeholder.dept',
  'ex.field.fromDate', 'ex.field.toDate', 'ex.field.postFamily', 'ex.field.shiftNo', 'cai.field.category',
  'ex.opt.all', 'ex.btn.search', 'ex.btn.export',
  'ex.col.no', 'ex.col.empId', 'ex.col.fullName', 'ex.col.dept', 'ex.col.position',
  'cai.col.shift', 'cai.col.category', 'cai.col.duration', 'cai.col.workDate', 'cai.col.fromTime', 'cai.col.toTime',
...
/**
 * Bản Angular của ess/infoApplyAttendance/viewCoordApplyAttendanceInfoList.html (Thymeleaf + DataTables
 * serverSide) - Tra cứu chấm công của nhân viên cấp dưới cho quản lý. Gọi lại nguyên các API JSON đã có
 * sẵn ở EssInfoApplyAttendanceController (xem CoordApplyAttendanceInfoService) - không đổi backend.
 * Kiến trúc bám theo DeptOtApplyInfoComponent (nz-table phân trang server-side qua nzFrontPagination=false
 * + DataTablesResponse<T>) và AttendanceExBatchInfoComponent/CheckAttendanceExForBatchComponent
 ...
 */
@Component({
  selector: 'app-coord-apply-attendance-info',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './coord-apply-attendance-info.component.html',
  styleUrl: './coord-apply-attendance-info.component.css',
})
export class CoordApplyAttendanceInfoComponent implements OnInit {
  readonly pageSizeOptions = [25, 50, 100, 200, 500];

  readonly rows = signal<CoordApplyAttendanceRow[]>([]);
  ...
  readonly itemOptions = signal<AttendanceItemOption[]>([]);
  ...
  constructor(
    private readonly api: CoordApplyAttendanceInfoService,
  ...
  exportExcel(): void {
    ...
    const headers = [
      ...
      t('cai.col.category', 'Phân loại'), t('cai.col.duration', 'Thời lượng'), t('cai.col.workDate', 'Ngày công'),
      t('cai.col.fromTime', 'Thời gian bắt đầu'), t('cai.col.toTime', 'Thời gian kết thúc'),
    ];
    const dataRows = this.rows().map((r, i) => [
      i + 1, r.empId, r.localName, r.deptName, r.postGradeName, r.shiftName, r.itemName,
      `${r.quantity || ''} ${r.unit || ''}`.trim(), r.arDateStr, r.fromDate, r.toTime,
    ]);
