...
import { ViewEntryInfoListService } from './view-entry-info-list.service';
import { AuthDeptNode, EssEntryInfoListDto, ShiftOption } from './view-entry-info-list.model';

/** Toàn bộ key bên dưới đã có sẵn trong message.properties (namespace "acrd" dùng riêng cho trang
 *  này, "common"/"attSearch"/"acr" dùng chung với các trang khác) - không cần thêm key mới. */
const I18N_KEYS = [
  'common.search.empIdOrName', 'common.placeholder.enterEmpId', 'common.fromDate', 'common.toDate',
  'common.deptName', 'common.select', 'attSearch.shiftType', 'attSearch.all',
  'acrd.col.missingCard', 'acrd.missingCard.all', 'acrd.missingCard.yes', 'acrd.missingCard.no',
  'common.search', 'common.clearFilter', 'common.exportExcel',
  'common.stt', 'common.empId', 'common.empName', 'acrd.col.team', 'common.position',
  'attSearch.workDate', 'acrd.col.inDay', 'acrd.col.inTime', 'acrd.col.outDay', 'acrd.col.outTime',
  'acr.col.remark', 'acrd.col.shiftName', 'acrd.col.changeShiftPerson', 'acrd.col.eatTime',
  'common.loadFail', 'common.noData', 'common.totalRows', 'mep.msg.loadDeptFailed',
];

/** Mã trạng thái thiếu thẻ (MISSING_CARD) - đúng giá trị hardcode ở select bản gốc (1=thiếu, 2=đủ). */
const MISSING_CARD_OPTIONS: { value: string; labelKey: string; fallback: string }[] = [
  { value: '1', labelKey: 'acrd.missingCard.yes', fallback: 'Thiếu thẻ' },
  { value: '2', labelKey: 'acrd.missingCard.no', fallback: 'Đủ thẻ' },
];

/**
 * Bản Angular của ess/viewDept/viewEntryInfoList.html (Thymeleaf + DataTables serverSide) - Tra cứu dữ
 * liệu quẹt thẻ theo ca làm (chỉ đọc). Gọi lại nguyên API JSON đã có sẵn ở EssViewDeptController (xem
 * ViewEntryInfoListService) - không đổi backend. Kiến trúc bám theo CoordApplyAttendanceInfoComponent
 ...
 * đúng hành vi DataTables Buttons gốc không cấu hình exportOptions.modifier.page='all'). Bỏ nút kính lúp
 * mở EmployeeSearchModal.js cạnh ô từ khóa (bản gốc) vì ô từ khóa đã gõ-tìm trực tiếp được, giống mọi
 * trang tra cứu khác đã migrate (dept-ot-apply-info, coord-apply-attendance-info) không có nút này.
 */
@Component({
  selector: 'app-view-entry-info-list',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './view-entry-info-list.component.html',
  styleUrl: './view-entry-info-list.component.css',
})
export class ViewEntryInfoListComponent implements OnInit {
  readonly pageSizeOptions = [25, 50, 100, 200, 500];
  readonly missingCardOptions = MISSING_CARD_OPTIONS;

  readonly rows = signal<EssEntryInfoListDto[]>([]);
  ...
  constructor(
    private readonly api: ViewEntryInfoListService,
  ...
  shiftLabel(row: EssEntryInfoListDto): string {
  ...
  exportExcel(): void {
    ...
    const dataRows = this.rows().map((r, i) => [
      i + 1, r.empId, r.localName, r.deptTeam, r.deptName, r.postGradeNoName,
      r.arDateStr, r.inDay, r.inTime, r.outDay, r.outTime, r.leaveContent,
      this.shiftLabel(r), r.changeShiftPerson, r.eatTimes,
    ]);
