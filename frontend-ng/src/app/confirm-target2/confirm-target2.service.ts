 ...
 * viewConfirmTarget2.html đang dùng) - không đổi backend. getItemList dùng chung endpoint
 * /personalTarget/itemList với RegPersonalTargetComponent/ConfirmTarget1Component (chỉ đọc).
 */
@Injectable({ providedIn: 'root' })
export class ConfirmTarget2Service {
  ...
  getObjectList(params: {
    ...
    return this.http.get<DataTablesResponse<EvsConfirmTarget1Row>>(`${API_BASE}/confirmTarget2/objectList`, {
      params: httpParams,
      withCredentials: true,
    });
  }
  ...
  getObjectInfo(evsObjectSeq: string): Observable<EvsPersonalTargetInfo> {
    return this.http.get<EvsPersonalTargetInfo>(`${API_BASE}/confirmTarget2/objectInfo`, {
  ...
  confirm(payload: ConfirmPayload): Observable<ApiResult> {
    return this.http.post<ApiResult>(`${API_BASE}/confirmTarget2/confirm`, payload, { withCredentials: true });
  }
}
