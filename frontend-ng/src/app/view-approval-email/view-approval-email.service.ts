...
import { ApplyDetailResponse, ExecuteItem, ExecuteResult, SyAffirmEmailDto } from './view-approval-email.model';

const API_BASE = '/ess/infoApply/api/approvalEmail';
...
/**
 * Gọi lại nguyên các API JSON đã có sẵn ở backend mà viewApprovalEmail.html (Thymeleaf) và 3 modal chi
 * tiết dùng chung (viewApprovaledOt/viewApprovaledLeave/viewAttendanceEx) đang dùng - không đổi backend.
 */
@Injectable({ providedIn: 'root' })
export class ViewApprovalEmailService {
  ...
  execute(items: ExecuteItem[]): Observable<ExecuteResult> {
    return this.http.post<ExecuteResult>(`${API_BASE}/execute`, { items }, { withCredentials: true });
  }
